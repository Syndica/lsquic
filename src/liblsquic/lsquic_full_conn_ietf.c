                ...
                lsquic_time_t now)
{
    ...
    fprintf(stderr, "ACKDBG gen_ack_for_pns: pns=%d avail=%hu now=%"PRIu64"\n",
            pns, lsquic_packet_out_avail(packet_out), (uint64_t)now);
    w = conn->ifc_conn.cn_pf->pf_gen_ack_frame(
            ...
            ecn_counts);
    fprintf(stderr, "ACKDBG gen_ack_for_pns: result w=%d\n", w);
    if (w < 0) {
