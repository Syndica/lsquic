...
X509 *
create_dummy_x509_cert(uint8_t public_key[32], uint8_t private_key[64]) {
...
prog_init_ssl_ctx (struct prog *prog)
{
    ...
    X509 *cert = create_dummy_x509_cert(public_key, private_key);
    if (cert == NULL)
    {
        LSQ_ERROR("cannot create dummy X509 certificate");
        return -1;
    }
    ...
    if (0 == SSL_CTX_use_PrivateKey(prog->prog_ssl_ctx, pkey)) {
        LSQ_ERROR("SSL_CTX_use_PrivateKey failed");
        return -1;
    }

    if (0 == SSL_CTX_use_certificate(prog->prog_ssl_ctx, cert)) {
        LSQ_ERROR("SSL_CTX_use_certificate failed");
        return -1;
    }
